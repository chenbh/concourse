...
var _ = Describe("Fly CLI", func() {
	Describe("unpause-pipeline", func() {
		Context("when the pipeline name is specified", func() {
			...
			BeforeEach(func() {
				mainPath, err = types.Routes.CreatePathForRoute(types.UnpausePipeline, rata.Params{"pipeline_name": "awesome-pipeline", "team_name": "main"})
				Expect(err).NotTo(HaveOccurred())

				otherPath, err = types.Routes.CreatePathForRoute(types.UnpausePipeline, rata.Params{"pipeline_name": "awesome-pipeline", "team_name": "other-team"})
				Expect(err).NotTo(HaveOccurred())
			})
	...
	Context("when the --all flag is passed", func() {
		...
		BeforeEach(func() {
			somePath, err = types.Routes.CreatePathForRoute(types.UnpausePipeline, rata.Params{"pipeline_name": "awesome-pipeline", "team_name": "main"})
			Expect(err).NotTo(HaveOccurred())

			someOtherPath, err = types.Routes.CreatePathForRoute(types.UnpausePipeline, rata.Params{"pipeline_name": "more-awesome-pipeline", "team_name": "main"})
			Expect(err).NotTo(HaveOccurred())
