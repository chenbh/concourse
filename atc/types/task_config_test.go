...
var _ = Describe("TaskConfig", func() {
	Describe("validating", func() {
		var (
			invalidConfig types.TaskConfig
			validConfig   types.TaskConfig
		)
...
run: {path: a/file}
`)
					task, err := types.NewTaskConfig(data)
...
run: {path: a/file}
`)
					config, err := types.NewTaskConfig(data)
					Expect(err).ToNot(HaveOccurred())
...
`)
					...
					Expect(err).ToNot(HaveOccurred())
					Expect(task.Platform).To(Equal("beos"))
					Expect(task.Params).To(Equal(types.TaskEnv{"FOO": "1"}))
				})
			})
...
run: {path: a/file}
`)
					_, err := types.NewTaskConfig(data)
...
`)
					...
					cpu := uint64(1024)
					memory := uint64(1024)
					Expect(task.Limits).To(Equal(&types.ContainerLimits{
						CPU:    &cpu,
						Memory: &memory,
					}))
				})
...
`)
		...
		Context("when the task has inputs", func() {
			BeforeEach(func() {
				validConfig.Inputs = append(validConfig.Inputs, types.TaskInputConfig{Name: "concourse"})
			})
			...
			Context("when input.name is missing", func() {
				BeforeEach(func() {
					invalidConfig.Inputs = append(invalidConfig.Inputs, types.TaskInputConfig{Name: "concourse"}, types.TaskInputConfig{Name: ""})
				})
			...
			Context("when input.name is missing multiple times", func() {
				BeforeEach(func() {
					invalidConfig.Inputs = append(
						...
						types.TaskInputConfig{Name: "concourse"},
						types.TaskInputConfig{Name: ""},
						types.TaskInputConfig{Name: ""},
					)
				})
		...
		Context("when the task has outputs", func() {
			BeforeEach(func() {
				validConfig.Outputs = append(validConfig.Outputs, types.TaskOutputConfig{Name: "concourse"})
			})
			...
			Context("when output.name is missing", func() {
				BeforeEach(func() {
					invalidConfig.Outputs = append(invalidConfig.Outputs, types.TaskOutputConfig{Name: "concourse"}, types.TaskOutputConfig{Name: ""})
				})
			...
			Context("when output.name is missing multiple times", func() {
				BeforeEach(func() {
					invalidConfig.Outputs = append(
						...
						types.TaskOutputConfig{Name: "concourse"},
						types.TaskOutputConfig{Name: ""},
						types.TaskOutputConfig{Name: ""},
					)
				})
